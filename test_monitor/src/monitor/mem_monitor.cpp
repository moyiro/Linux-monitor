...
void MemMonitor::UpdateOnce(monitor::proto::MonitorInfo* monitor_info) {
  ...
  mem_detail->set_used_percent((mem_info.total - mem_info.avail) *
                               1.0 /  // 内存利用率
                               mem_info.total * 100.0);
  mem_detail->set_total(mem_info.total / KBToGB);  // 总内存
  mem_detail->set_free(mem_info.free / KBToGB);    // 空闲内存
  mem_detail->set_avail(mem_info.avail / KBToGB);
  mem_detail->set_buffers(mem_info.buffers / KBToGB);  // 块设备占用的缓存页
  mem_detail->set_cached(mem_info.cached / KBToGB);  // 普通文件数据占用的缓存页
  mem_detail->set_swap_cached(mem_info.swap_cached /
                              KBToGB);  // 确定要被换页，但为被写进内存中的
